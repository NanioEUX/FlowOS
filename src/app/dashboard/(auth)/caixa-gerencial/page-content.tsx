...
const typeColors: Record<string, string> = { sale: "text-green-600", expense: "text-red-400", withdrawal: "text-red-400", injection: "text-green-600", refund: "text-amber-400" }
      ...
      <div className="flex gap-2">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${tab === t.key ? "bg-green-600 text-white" : "bg-zinc-100 text-zinc-400 hover:bg-white/[.08]"}`}
          >
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(["today", "7days", "30days", "all"] as DateFilter[]).map((f) => (
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${dateFilter === f ? "bg-green-600 text-white" : "bg-zinc-100 text-zinc-400 hover:bg-white/[.08]"}`}
          >
        ...
        <button
          ...
          className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${dateFilter === "custom" ? "bg-green-600 text-white" : "bg-zinc-100 text-zinc-400 hover:bg-white/[.08]"}`}
        >
          <Filter className="h-3 w-3" />
        </button>
        {dateFilter === "custom" && (
          <div className="flex items-center gap-2">
            <input type="date" value={customStart} onChange={(e) => setCustomStart(e.target.value)} className="flex h-10 w-36 items-center rounded-lg border border-zinc-200 bg-zinc-50 px-3 text-xs text-zinc-700 placeholder:text-zinc-400 focus:border-green-600 focus:outline-none" />
            <span className="text-xs text-zinc-400">até</span>
            <input type="date" value={customEnd} onChange={(e) => setCustomEnd(e.target.value)} className="flex h-10 w-36 items-center rounded-lg border border-zinc-200 bg-zinc-50 px-3 text-xs text-zinc-700 placeholder:text-zinc-400 focus:border-green-600 focus:outline-none" />
          </div>
        )}
      </div>
      ...
      {tab === "atual" && (
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <Card className="border-green-600/20 bg-green-600/10">
              <CardContent className="p-4 text-center">
                <p className="text-xs text-green-600 mb-1">Suprimentos</p>
                <p className="text-xl font-bold text-green-600">{formatCurrency(totalInjection)}</p>
              </CardContent>
            </Card>
          ...
          {cashRegister && (
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="outline" className="gap-1.5 text-green-600 border-green-600/20 hover:bg-green-600/10" onClick={() => { setMovAmount(""); setSuprimentoModal(true) }}>
                    ...
                    <CardContent className="p-4">
                      <h4 className="flex items-center gap-2 text-sm font-semibold text-zinc-700 mb-2">
                        {type === "sale" ? <TrendingUp className="h-4 w-4 text-green-600" /> :
                         type === "injection" ? <Plus className="h-4 w-4 text-green-600" /> :
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="grid grid-cols-3 gap-3 mb-3">
                  ...
                  <div className="rounded-lg bg-zinc-50 p-3 text-center">
                    <p className="text-lg font-bold text-green-600">{formatCurrency(cardSales)}</p>
          ...
          {historyLoading ? (
            ...
            <Card>
              <CardContent className="p-8 text-center text-zinc-400">
                <History className="h-8 w-8 mx-auto mb-2 text-zinc-700" />
            ...
            <>
              <div className="space-y-3">
                {historyRegisters.map((reg: any) => (
                  <Card key={reg.id} className={reg.status === "open" ? "border-green-600/20" : ""}>
                    <CardContent className="p-4">
                      <button
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${reg.status === "open" ? "bg-green-600/10" : "bg-zinc-100"}`}>
                            {reg.status === "open"
                          ...
                          <div className="text-left">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${reg.status === "open" ? "bg-green-600/10 text-green-600" : "bg-zinc-100 text-zinc-500"}`}>
                      ...
                      {expandedRegister === reg.id && (
                        <div className="mt-4 border-t border-white/[.04] pt-4 space-y-3">
                          <div className="grid grid-cols-3 gap-2 text-center text-xs">
                            ...
                            <div className="rounded-lg bg-zinc-50 p-2">
                              <p className={`font-bold ${(reg.closingAmount || 0) >= (reg.expectedAmount || 0) ? "text-green-600" : "text-red-400"}`}>
                            ...
                            <div className="space-y-1.5 max-h-60 overflow-y-auto">
                              {expandedMovements[reg.id].map((m: any) => (
                                <div key={m.id} className="flex items-center justify-between rounded-lg bg-zinc-50 px-3 py-2 text-sm">
                                  ...
                                  <span className={`font-semibold ${m.amount >= 0 ? "text-green-600" : "text-red-400"}`}>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <Card className="border-green-600/20 bg-green-600/10">
                  <CardContent className="p-4 text-center">
                    <p className="text-xs text-green-600 mb-1">Suprimentos</p>
                    <p className="text-xl font-bold text-green-600">{formatCurrency(summary.totalInjections)}</p>
                  </CardContent>
                </Card>
                <Card className="border-amber-500/20 bg-amber-500/10">
                  <CardContent className="p-4 text-center">
                    <p className="text-xs text-amber-400 mb-1">Líquido</p>
                    <p className={`text-xl font-bold ${summary.netResult >= 0 ? "text-green-600" : "text-red-400"}`}>{formatCurrency(summary.netResult)}</p>
                  </CardContent>
                </Card>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            ...
            <input
              ...
              autoFocus
              className="mb-4 flex h-10 w-full items-center rounded-lg border border-zinc-200 bg-zinc-50 px-3 text-sm text-zinc-700 placeholder:text-zinc-400 focus:border-green-600 focus:outline-none"
            />
            <div className="flex gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            ...
            <input
              ...
              autoFocus
              className="mb-3 flex h-10 w-full items-center rounded-lg border border-zinc-200 bg-zinc-50 px-3 text-sm text-zinc-700 placeholder:text-zinc-400 focus:border-green-600 focus:outline-none"
            />
            <input
              type="number"
              placeholder="Valor"
