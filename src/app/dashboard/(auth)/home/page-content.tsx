                ...
                return (
                  <div key={day.date} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 h-6 bg-zinc-100 rounded-md overflow-hidden">
                      <div
                        className={`h-full rounded-md transition-all ${isToday ? "bg-green-600" : "bg-zinc-300"}`}
              ...
              <div className="space-y-3">
                {data.topProducts.map((product, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-green-600/10 text-xs font-bold text-green-600">
                      {i + 1}
                    </span>
          ...
          <div className="space-y-2">
            {data.recentOrders.map((order) => {
              ...
              const statusColors: Record<string, string> = {
                ...
                out_for_delivery: "bg-green-600/15 text-green-600",
                delivered: "bg-green-600/10 text-green-600",
                cancelled: "bg-red-500/15 text-red-400",
              }
              return (
                <div key={order.id} className="flex items-center justify-between rounded-lg border border-white/[.04] bg-zinc-50 px-3 py-2">
                  ...
                  <div className="text-right">
                    ...
                    <span className={`inline-block rounded-full px-1.5 py-0.5 text-[10px] font-medium ${statusColors[order.status] || "bg-zinc-100 text-zinc-400"}`}>
