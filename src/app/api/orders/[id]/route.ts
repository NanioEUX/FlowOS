...
export async function PATCH(
  ...
  try {
    ...
    // - Cliente (anônimo via token): SÓ pode cancelar enquanto pedido ainda
    //   não foi aceito (status="pending"/"payment_pending").
    // - Merchant (admin autenticado): pode cancelar QUALQUER status não-final.
    if (status === "cancelled") {
      const isMerchant = !!auth
      if (!isMerchant) {
    ...
    const updated = await prisma.order.update({
      ...
      data: {
        ...
        ...(status === "out_for_delivery" && order.method !== "ifood" && !order.deliveryCode && { deliveryCode: order.customerPhone ? order.customerPhone.replace(/\D/g, "").slice(-4) : String(Math.floor(1000 + Math.random() * 9000)) }),
