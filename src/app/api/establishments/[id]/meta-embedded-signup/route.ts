...
export async function POST(
  ...
  try {
    ...
    if (existingToken) {
      console.log("[Meta Embedded Signup] Using existing token, length:", existingToken.length, "starts with:", existingToken.substring(0, 10))
    ...
    // Step 6: Register phone number with WhatsApp Cloud API (best effort)
    if (phoneNumberId) {
